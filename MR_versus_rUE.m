% MR versus r_UE in the case Kr=Ku=6
clear all;
figure(1);
addpath([pwd,'\func']);
...
radius_ue_List=[5,10,15,20,25,30,35];
minR_move=zeros(length(radius_ue_List),1);
minR_PhaseOnly=zeros(length(radius_ue_List),1);
minR_BenchMark=zeros(length(radius_ue_List),1);
minR_UpBound=zeros(length(radius_ue_List),1);
minR_Case1=zeros(length(radius_ue_List),1);
for r_idx=1:length(radius_ue_List)
    radius_ue=radius_ue_List(r_idx);
...
%         [C_MIMO]=getC_MIMO(A_r_tilde, Nr, Kr, Ku, Nbs, SNR);
        ...
        minR_move(r_idx) = minR_move(r_idx) + MR_PhaseOnly/MC_times;
        minR_PhaseOnly(r_idx) = minR_PhaseOnly(r_idx) + MR_Move/MC_times;
        minR_BenchMark(r_idx) = minR_BenchMark(r_idx) + MR_BenchMark/MC_times;
        minR_UpBound(r_idx) = minR_UpBound(r_idx) + upbound/MC_times;
        minR_Case1(r_idx) = minR_Case1(r_idx) + Case1/MC_times;
    end
    stop=1;
end
figure;
plot(radius_ue_List,minR_move,'linewidth',1.5,'Marker','o');hold on;
plot(radius_ue_List,minR_PhaseOnly,'linewidth',1.5,'Marker','v');hold on;
plot(radius_ue_List,minR_BenchMark,'linewidth',1.5,'Marker','s');hold on;
plot(radius_ue_List,minR_UpBound,'linewidth',1.5,'Marker','^');hold on;
plot(radius_ue_List,minR_Case1,'linewidth',1.5,'Marker','>');hold on;
xlabel('r_{ue}');
ylabel('MR (bps/Hz)');
