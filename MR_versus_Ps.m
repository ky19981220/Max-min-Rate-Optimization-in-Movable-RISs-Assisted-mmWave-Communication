% MR versus Ps in the case Kr=Ku=6
clear all;
figure(1);
addpath([pwd,'\func']);
...
Kr=6;
Ku=Kr;
SNR_dB_List=[140:5:160]; % dB
minR_move=zeros(length(SNR_dB_List),1);
minR_PhaseOnly=zeros(length(SNR_dB_List),1);
minR_BenchMark=zeros(length(SNR_dB_List),1);
minR_UpBound=zeros(length(SNR_dB_List),1);
minR_Case1=zeros(length(SNR_dB_List),1);
sumR_BenchMark=zeros(length(SNR_dB_List),1);
...
%         [C_MIMO]=getC_MIMO(A_r_tilde, Nr, Kr, Ku, Nbs, SNR);
        ...
        minR_move(SNR_idx) = minR_move(SNR_idx) + MR_Move/MC_times;
        minR_PhaseOnly(SNR_idx) = minR_PhaseOnly(SNR_idx) + MR_PhaseOnly/MC_times;
        minR_BenchMark(SNR_idx) = minR_BenchMark(SNR_idx) + MR_BenchMark/MC_times;
        minR_UpBound(SNR_idx) = minR_UpBound(SNR_idx) + upbound/MC_times;
        minR_Case1(SNR_idx) = minR_Case1(SNR_idx) + Case1/MC_times;
        sumR_BenchMark(SNR_idx)= sumR_BenchMark(SNR_idx) + SR_BenchMark/MC_times;
    end
    stop=1;
end
figure;
plot(SNR_dB_List,minR_move,'linewidth',1.5,'Marker','o');hold on;
plot(SNR_dB_List,minR_PhaseOnly,'linewidth',1.5,'Marker','v');hold on;
plot(SNR_dB_List,minR_BenchMark,'linewidth',1.5,'Marker','s');hold on;
plot(SNR_dB_List,minR_UpBound,'linewidth',1.5,'Marker','^');hold on;
plot(SNR_dB_List,minR_Case1,'linewidth',1.5,'Marker','>');hold on;
xlabel('P_s');
ylabel('MR (bps/Hz)');
